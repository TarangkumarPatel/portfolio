...
export async function POST(request) {
  ...
  const { name, title, organization, relationship, relationshipOther, message, email, linkedinUrl } = data;
  ...
  const resolvedRelationship = RELATIONSHIP_OPTIONS.includes(relationship) ? relationship : 'Other';
  if (resolvedRelationship === 'Other' && !relationshipOther?.trim()) {
    return NextResponse.json({ error: 'Please specify your relationship.' }, { status: 400 });
  }
  ...
  const doc = {
    ...
    relationshipOther: resolvedRelationship === 'Other' ? relationshipOther.trim().slice(0, 150) : '',
